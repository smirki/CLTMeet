...
def collect_contents(directory, output_file, skip_dirs, skip_files):
    ...
    with open(output_file, 'w') as out_file:
        for root, dirs, files in os.walk(directory):
            # Modify the dirs list in-place to skip directories
            dirs[:] = [d for d in dirs if not any(os.path.join(root, d).startswith(skip_dir) for skip_dir in skip_dirs)]
...
if __name__ == "__main__":
    ...
    skip_directories = [
        ...
        os.path.join(start_directory, '.expo')
    ]
